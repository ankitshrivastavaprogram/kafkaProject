...
import com.ankit.admin.response.model.PendingJob;



public interface ServiceCounterDetailRepository extends JpaRepository<ServiceCounterDetail,Long>{
	
	@Query("select NEW com.ankit.admin.response.model.PendingJob(s.jobAndStatus.id,s.jobAndStatus.tokenNumber,s.jobAndStatus.status.status)"
