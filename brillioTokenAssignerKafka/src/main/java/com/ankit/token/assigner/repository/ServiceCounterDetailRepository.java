...
public interface ServiceCounterDetailRepository extends JpaRepository<ServiceCounterDetail, Long> {
	 ...
	 * tokenType);
	 * 
	 * select NEW ServiceCounterTaskCount(c.counter.id,count(*)) from
	 * ServiceCounterDetail c where c.servingTokenType.id = :tokenType group by
	 * c.counter.id
	 */
	 ...
	 * 
	 * select c from ServiceCounterDetail as c where c.servingTokenType =1 AND
	 * c.jobAndStatusList is null
	 */

	@Query("select c from ServiceCounterDetail c where c.servingTokenType.id = :tokenType AND c.jobAndStatus is null")
	List<ServiceCounterDetail> findCountersWithNoTaskAsignYetByTokenType(@Param("tokenType") Long tokenType,
			Pageable pageable);

	@Query("select NEW com.ankit.token.assigner.model.ServiceCounterTaskCount(c.counter.id,count(c.counter.id)) "
	...
	List<ServiceCounterTaskCount> findCountersWithMinmumPendingTaskByTokenType(
			@Param("servingTokenType") Long servingTokenType, @Param("jobStatus") Long jobStatus);
